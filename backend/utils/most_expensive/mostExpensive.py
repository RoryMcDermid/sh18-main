import requests
from get_wholesale_energy_price import *
    ...
    for system in all_systems:
        ...
        for sensor in all_sensors:
            ...
            oldest_date_formatted = dt.datetime.strftime(
                dt.datetime.strptime(formatted_timestamps[0, 0], "%Y-%m-%dT%H:%M:%S"), "%d-%m-%Y"
            )
            most_recent_date_formatted = dt.datetime.strftime(
                dt.datetime.strptime(formatted_timestamps[-1, -1], "%Y-%m-%dT%H:%M:%S"), "%d-%m-%Y"
            )
