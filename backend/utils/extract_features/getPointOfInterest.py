...
def get_points_of_interest(priority: List[float]):
    # find the maximum point of the graph
    maxEdges = [-1, -1]
    if priority[max(priority)] > 0:
        maxEdges = find_spike_or_trough(priority, max(priority), 1)

    # find the minimum point of the graph
    minEdges = [-1, -1]
    if priority[min(priority)] < 0:
        minEdges = find_spike_or_trough(priority, min(priority), -1)
...
def find_spike_or_trough(array, pointOfInterest, direction):
    pointLeft = 0
